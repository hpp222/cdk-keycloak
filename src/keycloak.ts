...
export class ContainerService extends Construct {
  ...
  constructor(scope: Construct, id: string, props: ContainerServiceProps) {
    ...
    let entryPoint = fs.readFileSync('src/entryPoint.txt', 'utf8').split(',');
    ...
    if (isQuarkusDistribution) {
      ...
      environment = {
        ...
        KC_PROXY: 'edge',
        INITIALILIZE_SQL: 'CREATE TABLE IF NOT EXISTS JGROUPSPING (own_addr varchar(200) NOT NULL, cluster_name varchar(200) NOT NULL, ping_data VARBINARY(255), constraint PK_JGROUPSPING PRIMARY KEY (own_addr, cluster_name));',
